...
export class AiController {
	...
	@ApiResponse({ status: 200, description: 'Thành công', type: AnalyzeFoodResponseDto })
	@UseInterceptors(
		FileInterceptor('file', {
			limits: { fileSize: 5 * 1024 * 1024 },
			fileFilter: (_request, file: any, callback: (error: Error | null, acceptFile: boolean) => void) => {
				if (!file.mimetype.match(/\/(jpg|jpeg|png|webp)$/)) {
					return callback(new BadRequestException('Chỉ chấp nhận ảnh (jpg, png, webp)'), false);
				}
				callback(null, true);
			},
		}),
	...
	async analyzeFood(
		...
		const userAllergies = user.allergies?.map((allergy) => allergy.allergen_code) ?? [];
