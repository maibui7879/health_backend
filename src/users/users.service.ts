...
import { ActivityLevel, GoalType, UpdateProfileDto } from './dto/update-profile.dto';
...
export class UsersService {
  ...
  async updateProfile(userId: string, dto: UpdateProfileDto) {
    ...
    if (
      ...
      let bmr = 10 * profile.current_weight_kg + 6.25 * profile.height_cm - 5 * age;
      ...
      const tdee =
        bmr * (activityMultipliers[profile.activity_level || ActivityLevel.SEDENTARY]);

      let adjustedTdee = tdee;
      if (profile.goal_type === GoalType.LOSE_WEIGHT) adjustedTdee -= 500;
      if (profile.goal_type === GoalType.GAIN_MUSCLE) adjustedTdee += 300;
